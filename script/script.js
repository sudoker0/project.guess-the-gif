...
async function customFetch(input, init) {
    ...
    if (!(r === null || r === void 0 ? void 0 : r.ok) || not_good_at_all) {
        console.log(`-------------------------------------\n` +
            ...
            `Response: \n${await (r === null || r === void 0 ? void 0 : r.text())}\n` +
            `-------------------------------------`);
        console.log(r);
...
async function newgame() {
    ...
    if (reset_count >= max_reset) {
        tag_list = [];
        const r_featured = await customFetch(`https://tenor.googleapis.com/v2/featured` +
            `?key=${api_key}` +
            `&limit=50` +
    ...
    const r_gif_data = await customFetch(`https://tenor.googleapis.com/v2/search` +
        `?key=${api_key}` +
        `&q=${search_key()}`);
    if (r_gif_data == null)
        return false;
